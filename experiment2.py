...
class Experiment():
    ...
    def __init__(self, **args):
        ...
        self.model = OneStepRNN(hidden_size=self.hidden_size, input_size=self.input_dim,
                                recurrent_cell=self.recurrent_cell, output_size=self.output_size)
        ...
        self.init_hidden = torch.autograd.Variable(torch.zeros(1, self.hidden_size), requires_grad=True)
    ...
    def log_differences(self, differences):
        if self.log_difference:
            ...
            differences_dict = {
                ...
                "dW_ih": np.linalg.norm(tmp_log_differences_dict["dW_ih"] - self.log_differences_dict["dW_ih"],
                                        ord="fro"),
                "W_hh": np.linalg.norm(tmp_log_differences_dict["W_hh"] - self.log_differences_dict["W_hh"], ord="fro"),
                "dW_hh": np.linalg.norm(tmp_log_differences_dict["dW_hh"] - self.log_differences_dict["dW_hh"],
                                        ord="fro"),
            }
